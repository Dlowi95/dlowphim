...
| Console/build | Đạt | Backend qua 22/22 suite, 112/112 test và Nest production build. Frontend TypeScript thành công, 8/8 test episode/playback qua, 7/7 Playwright chạy trên `next start` qua và production build hoàn tất; `/watch/[slug]` là `21.9 kB`, First Load JS `147 kB`. |

Giới hạn còn lại: Chromium không mô phỏng được native fullscreen của Safari iPhone. Bản vá iOS đã qua TypeScript, 8/8 test watch, 7/7 Playwright production và production build (`/watch/[slug]` `21.9 kB`, First Load JS `147 kB`), nhưng cần xác nhận một lượt trên iPhone thật rằng Safari chrome biến mất, video dùng native controls và xoay dọc/ngang không làm Navbar desktop xuất hiện. E2E hiện kiểm chứng auth-loading ban đầu và ownership token/pending; thao tác đăng nhập rồi đổi trực tiếp sang một tài khoản thứ hai trong cùng tab chưa có fixture UI chuyên biệt.
