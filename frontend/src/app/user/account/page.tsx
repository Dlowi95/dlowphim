  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const size = 400; // Kích thước lý tưởng siêu nét cho avatar
        canvas.width = size;
        canvas.height = size;

        if (ctx) {
          // Crop vuông chính giữa
            ...
            <div className="relative group/avatar cursor-pointer" onClick={triggerFileSelect}>
              ...
              <div className="w-36 h-36 md:w-44 md:h-44 rounded-full overflow-hidden border-4 border-zinc-800 shadow-xl group-hover/avatar:border-pink-500 transition-all duration-300 relative bg-zinc-900">
                <img
                  src={avatar}
                  alt="Avatar Preview"
                  className="w-full h-full object-cover rounded-full avatar-smooth"
                ...
                {AVAILABLE_AVATARS.map((url, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative aspect-square rounded-2xl overflow-hidden border-2 cursor-pointer transition-all duration-200 bg-zinc-900/60 flex items-center justify-center p-2 ${isSelected
