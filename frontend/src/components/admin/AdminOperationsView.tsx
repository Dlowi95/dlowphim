...
export default function AdminOperationsView() {
  ...
  const enqueue = async (type: "movie_metadata_sync" | "movie_availability_scan" | "upcoming_reminder_scan" | "source_health_check") => {
  ...
  return <div className="space-y-5">
    ...
    {tab === "audit" ? <>
      ...
      <div className="grid gap-3 md:grid-cols-2"><JobButton title="Đồng bộ lịch chiếu TMDB" text="Cập nhật metadata phim chiếu rạp; không tạo bản phát giả." onClick={() => void enqueue("movie_metadata_sync")} /><JobButton title="Dò và ghép bản phát" text="Đối chiếu TMDB ID, tên và năm trên PhimAPI/OPhim." onClick={() => void enqueue("movie_availability_scan")} /><JobButton title="Gửi nhắc phim" text="Gửi thông báo khi đến lịch hoặc phim đã có bản xem." onClick={() => void enqueue("upcoming_reminder_scan")} /><JobButton title="Kiểm tra nguồn phim" text="Đo trạng thái PhimAPI và OPhim ở background." onClick={() => void enqueue("source_health_check")} /></div>
      <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0c0d13]">{loading ? <EmptyLoader /> : jobs.length === 0 ? <Empty text="Chưa có tác vụ nền" /> : jobs.map((job) => <div key={job._id} className="flex flex-wrap items-center gap-3 border-b border-white/[0.04] px-4 py-3 last:border-0"><Status status={job.status} /><div className="min-w-0 flex-1"><p className="text-xs font-bold text-zinc-200">{jobTitle(job.type)}</p><p className="mt-1 text-[10px] text-zinc-500">Lần chạy {job.attempts}/{job.maxAttempts} · {new Date(job.createdAt).toLocaleString("vi-VN")}</p>{job.errorMessage && <p className="mt-1 text-[10px] text-red-400">{job.errorMessage}</p>}</div>{job.status === "failed" && <button onClick={() => void retry(job._id)} className="rounded-lg border border-red-500/20 px-3 py-1.5 text-[10px] font-black text-red-400">Thử lại</button>}</div>)}</div>
    </>}
  </div>;
}
...
function jobTitle(type: string) { return ({ movie_metadata_sync: "Đồng bộ lịch chiếu TMDB", movie_availability_scan: "Dò và ghép bản phát", upcoming_reminder_scan: "Gửi nhắc phim", source_health_check: "Kiểm tra nguồn phim" } as Record<string, string>)[type] || type; }
